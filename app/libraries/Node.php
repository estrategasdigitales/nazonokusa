...
class Node{
    ...
    private function __do( $paths, $id, $j, $input, $_input, $template, $output, $pathParent = NULL ){
		...
		foreach ( $inputs as $i => $record ){
			if ( ! @array_key_exists( $i, $output ) && $j == 0 )
				$output[$i] = $template;
	...
	private function _toXML( $writer, $nodes, $parentKey, &$i = 0 ){
		foreach ( $nodes as $nKey => $nValue ){
			$key 	= $parentKey;
			$value 	= $nValue;
			if ( is_array( $nValue ) && count( $nValue ) > 0 ){
			...
			if ( ! is_array( $nValue ) && ! is_array( $key ) && ! empty( $nValue ) ){
				$writer->startElement( $key );
				$writer->text( $nValue );
				$writer->endElement();
			}
